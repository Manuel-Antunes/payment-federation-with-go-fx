...
func (ec *executionContext) resolveEntity(
	...
	case "Payment":
		...
		case "findPaymentByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPaymentByID(): %w`, err)
			}
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Order", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
...
func entityResolverNameForPayment(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Payment", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
